...
const CATEGORIES = {
  comida: { label: 'CO', color: '#111111' },
  transporte: { label: 'TR', color: '#333333' },
  ocio: { label: 'OC', color: '#555555' },
  salud: { label: 'SA', color: '#777777' },
  educacion: { label: 'ED', color: '#222222' },
  hogar: { label: 'HO', color: '#444444' },
  ropa: { label: 'RO', color: '#666666' },
  servicios: { label: 'SE', color: '#888888' },
  otros: { label: 'OT', color: '#999999' },
};

const STORAGE_KEYS = {
  expenses: 'cashtrack_expenses',
...
function initNavigation() {
  ...
  navBtns.forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      if (section === 'charts') updateCharts();
...
function renderExpensesList() {
  ...
  if (filterMonth) {
    filtered = expenses.filter((e) => e.date.startsWith(filterMonth));
  }
  ...
  html =
    `<div style="text-align:right;margin-bottom:8px;font-weight:600;font-size:13px;color:#777;">
        Total: <span style="color:#111;">-$${total.toFixed(2)}</span> (${filtered.length} gastos)
    </div>` + html;
...
function initFilters() {
  document
    ...
    .addEventListener('change', renderExpensesList);
  document
...
function initBudgetForm() {
  ...
  document.getElementById('budget-form').addEventListener('submit', (e) => {
    ...
    if (!month || income <= 0) {
      showToast('Ingrese un mes e ingreso válido');
      return;
    }
    ...
    budgets[month] = { income, categories: catBudgets };
    saveBudgets();
    renderBudgetsList();
    ...
    html += `
            <div class="budget-saved-item">
                ...
                <div class="budget-saved-detail">
                    Ingreso: $${b.income.toFixed(2)} | Presupuestado: $${totalBudget.toFixed(2)}
                </div>
            </div>
        `;
  });
  ...
  slices.forEach((s) => {
    ...
    legendHtml += `
            <div class="legend-item">
                ...
                ${getCategoryLabel(s.cat)}: $${s.val.toFixed(2)} (${pct}%)
            </div>
        `;
  });
...
function updateSummary() {
  ...
  if (totalBudgeted > 0) {
    ...
    progressText.textContent = `${pct.toFixed(1)}% del presupuesto utilizado ($${totalSpent.toFixed(2)} de $${totalBudgeted.toFixed(2)})`;
  } else {
      ...
      catHtml += `
                <div class="category-detail-item">
                    ...
                    <div class="cat-detail-amounts">
                        <div class="cat-detail-spent">$${spent.toFixed(2)}</div>
                        <div class="cat-detail-budget">${budgeted > 0 ? 'de $' + budgeted.toFixed(2) : 'sin ppto.'}</div>
                    </div>
                </div>
            `;
    });
    ...
    sortedExpenses.forEach((exp, i) => {
      ...
      topHtml += `
                <div class="top-expense-item">
                    ...
                    <span class="top-expense-amount">$${exp.amount.toFixed(2)}</span>
                </div>
            `;
    });
